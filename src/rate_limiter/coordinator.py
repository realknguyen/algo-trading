...
class GlobalRateCoordinator:
    ...
    def register_exchange(
        self, exchange: str, config: ExchangeLimitConfig, enable_sliding_window: bool = False
    ) -> RateLimiter:
        ...
        self._exchange_status[exchange] = ExchangeStatus(
            exchange=exchange, last_update=time.time(), is_healthy=True
        )
    ...
    async def _do_acquire(
        ...
        try:
            ...
            if wait:
                await asyncio.wait_for(self._concurrency_semaphore.acquire(), timeout=timeout)
            else:
            ...
            acquired = await self._limiters.acquire(
                exchange, path, method, weight=weight, priority=priority, timeout=timeout
            )
    ...
    def _adjust_rate_for_exchange(self, exchange: str, headers: RateLimitHeaders) -> None:
        ...
        limiter = self._limiters.get_limiter(exchange)
        if limiter:
            asyncio.create_task(limiter.record_rate_limit_hit(headers.retry_after))
    ...
    def set_rate_limit_callback(self, callback: Callable[[str, RateLimitHeaders], None]) -> None:
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "concurrent_requests": (self._max_concurrent - self._concurrency_semaphore._value),
        }
                            ...
                            if now - status.last_rate_limit_hit > cooldown:
                                ...
                                if status.adaptive_rate_multiplier < 1.0:
                                    status.adaptive_rate_multiplier = min(
                                        1.0, status.adaptive_rate_multiplier * 1.1
                                    )
