...
class RateLimitStateManager:
    ...
    async def save_token_bucket_state(
        self,
        exchange: str,
        bucket_name: str,
        bucket
    ...
    async def restore_token_bucket_state(
        ...
        tokens = min(
            data.get("tokens", 0) + elapsed * data.get("rate", 10),
    ...
    async def save_all_states(self, limiters: Dict[str, Any]) -> None:
        ...
        for exchange, limiter in limiters.items():
            ...
            if hasattr(limiter, "_global_bucket"):
                await self.save_token_bucket_state(
                    exchange, "global", limiter._global_bucket
                )
            
            # Save order bucket
            if hasattr(limiter, "_order_bucket"):
                await self.save_token_bucket_state(
                    exchange, "order", limiter._order_bucket
                )
