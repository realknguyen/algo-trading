...
from src.rate_limiter.token_bucket import TokenBucket, AdaptiveTokenBucket, Priority, TokenBucketGroup
...
class InMemoryBackend(RateLimiterBackend):
    ...
    def _cleanup_expired(self, now: float) -> None:
        ...
        for key in self._windows:
            self._windows[key] = [
                (ts, exp) for ts, exp in self._windows[key]
                if now <= exp
            ]
