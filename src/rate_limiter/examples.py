...
async def example_adaptive_bucket():
    ...
    bucket = AdaptiveTokenBucket(
        rate=100.0, capacity=200.0, min_rate=10.0, max_rate=500.0, name="adaptive_api"
    )
...
async def example_exchange_limiter():
    ...
    config = ExchangeLimitConfig(
        ...
        endpoints=[
            ...
            EndpointConfig(pattern=r"/api/v3/account", rate=2.0, burst=5.0, weight=20),
        ],
    )
...
async def example_coordinator():
    ...
    binance_config = ExchangeLimitConfig(
        exchange_name="binance", global_rate=20.0, weight_limit=1200, weight_window=60.0
    )
    coordinator.register_exchange("binance", binance_config)

    # Configure Coinbase
    coinbase_config = ExchangeLimitConfig(
        exchange_name="coinbase", global_rate=10.0, global_burst=20.0
    )
    ...
    acquired = await coordinator.acquire(
        "binance", "/api/v3/order", "POST", weight=10, priority=Priority.HIGH
    )
    print(f"Binance order request acquired: {acquired}")
    coordinator.release("binance", "/api/v3/order", "POST")

    # Request to Coinbase
    acquired = await coordinator.acquire("coinbase", "/products", "GET", priority=Priority.NORMAL)
...
async def example_context_manager():
    ...
    # Register exchange
    config = ExchangeLimitConfig(exchange_name="example", global_rate=10.0)
    ...
    try:
        async with coordinator.request(
            "example", "/api/critical", "POST", weight=5, priority=Priority.CRITICAL
        ):
...
async def example_multi_exchange():
    ...
    # Add exchanges
    multi.add_exchange(ExchangeLimitConfig(exchange_name="binance", global_rate=20.0))

    multi.add_exchange(ExchangeLimitConfig(exchange_name="coinbase", global_rate=10.0))

    multi.add_exchange(ExchangeLimitConfig(exchange_name="kraken", global_rate=5.0))
...
async def example_priority_queue():
    ...
    async def make_request(name: str, priority: Priority):
        try:
            async with coordinator.request(
                "priority_test", "/api/action", "POST", priority=priority, timeout=5.0
            ):
