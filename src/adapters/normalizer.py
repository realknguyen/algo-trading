...
class OrderBook:
    ...
    def __post_init__(self):
        ...
        if self.bids and isinstance(self.bids[0], (list, tuple)):
            self.bids = [OrderBookLevel(Decimal(str(p)), Decimal(str(q))) 
                        for p, q in self.bids]
        if self.asks and isinstance(self.asks[0], (list, tuple)):
            self.asks = [OrderBookLevel(Decimal(str(p)), Decimal(str(q))) 
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        return {
            'bids': [{'price': str(l.price), 'quantity': str(l.quantity)} 
                    for l in self.bids],
            'asks': [{'price': str(l.price), 'quantity': str(l.quantity)} 
                    for l in self.asks],
            'timestamp': self.timestamp,
            'symbol': self.symbol,
            'exchange': self.exchange.value,
...
class Ticker:
    ...
    def __post_init__(self):
        """Ensure proper types after initialization."""
        for attr in ['bid', 'ask', 'last', 'volume', 'change_pct', 'high_24h', 'low_24h', 'quote_volume']:
