...
class HealthMetrics:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.status == ConnectionStatus.CONNECTED
            and self.connection_success_rate >= 0.8
            and (self.latency_ms is None or self.latency_ms < 5000)  # < 5s latency
        )
...
class ExchangeHealthMonitor:
    ...
    def register_exchange(
        ...
        """
        self.metrics[exchange] = HealthMetrics(
            exchange=exchange, status=ConnectionStatus.DISCONNECTED
        )
    ...
    async def connect(self, exchange: str) -> bool:
        ...
        try:
            ...
            if self.enable_circuit_breaker and exchange in self.circuits:
                connection = await self.circuits[exchange].call(self.connect_handlers[exchange])
            else:
    ...
    def get_health_status(self, exchange: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            "exchanges": {name: metrics.to_dict() for name, metrics in self.metrics.items()},
            "summary": {
                "total": len(self.metrics),
                "connected": sum(
                    1 for m in self.metrics.values() if m.status == ConnectionStatus.CONNECTED
                ),
    ...
    def get_circuit_status(self, exchange: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            "enabled": True,
            "exchanges": {name: circuit.to_dict() for name, circuit in self.circuits.items()},
        }
