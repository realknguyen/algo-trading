...
class LighterAdapter(BaseExchangeAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            raise ExchangeConnectionError(f"Connection failed: {e}", exchange=self.exchange_name)
    ...
    async def _get_next_nonce(self) -> int:
        ...
        response = await self._make_request("GET", "/api/transaction/next_nonce", params=params)
    ...
    def _sign_request(
        ...
        try:
            ...
            raise AuthenticationError(f"Signing failed: {e}", exchange=self.exchange_name)
    ...
    async def _make_public_request(
        ...
        async with self._rate_limiter:
            try:
                ...
                raise ExchangeConnectionError(f"Request failed: {e}", exchange=self.exchange_name)
            ...
            for attempt in range(retries):
                try:
                    ...
                    if isinstance(result, dict) and result.get("error"):
                        raise LighterError(
                            result.get("message", "Unknown error"), code=result.get("code")
                        )
                    ...
                    raise ExchangeConnectionError(
                        f"Connection failed: {e}", exchange=self.exchange_name
                    )
    ...
    def _handle_http_error(self, status_code: int, response: httpx.Response) -> None:
        ...
        if status_code == 401:
            raise AuthenticationError(
                f"Authentication failed: {message}", exchange=self.exchange_name
            )
        elif status_code == 429:
            raise LighterRateLimitError(
                f"Rate limit exceeded: {message}", exchange=self.exchange_name
            )
        elif status_code == 404:
            raise InvalidSymbolError(f"Resource not found: {message}", exchange=self.exchange_name)
        else:
            raise LighterError(f"HTTP {status_code}: {message}", code=code)
    ...
    async def get_account(self) -> AccountInfo:
        ...
        response = await self._make_request("GET", "/api/account", params=params, signed=True)
    ...
    async def get_balances(self) -> List[Balance]:
        ...
        response = await self._make_request(
            "GET", "/api/account/assets", params=params, signed=True
        )
        ...
        for asset_id, asset_data in assets.items():
            ...
            if free > 0 or locked > 0:
                balances.append(
                    Balance(asset=symbol, free=free, locked=locked, total=free + locked)
                )
    ...
    async def get_positions(self) -> List[Position]:
        ...
        response = await self._make_request(
            "GET", "/api/account/positions", params=params, signed=True
        )
        ...
        for market_id, pos_data in positions_data.items():
            symbol = self._market_id_to_symbol.get(int(market_id), f"MARKET_{market_id}")
    ...
    async def get_ticker(self, symbol: str) -> Ticker:
        ...
        if market_id is None:
            ...
            response = await self._make_public_request("GET", f"/ticker/{symbol}")
        else:
            # Use WebSocket API for real-time data
            params = {"market_index": market_id}
            response = await self._make_request("GET", "/api/market/stats", params=params)
    ...
    async def get_orderbook(self, symbol: str, limit: int = 100) -> OrderBook:
        ...
        if market_id is None:
            raise InvalidSymbolError(
                f"Unknown symbol: {symbol}", exchange=self.exchange_name, symbol=symbol
            )

        params = {"market_index": market_id}

        response = await self._make_request("GET", "/api/order/book_details", params=params)

        order_book = response.get("order_book", {})

        # Parse bids and asks
        bids = [[Decimal(b["price"]), Decimal(b["size"])] for b in order_book.get("bids", [])]
        asks = [[Decimal(a["price"]), Decimal(a["size"])] for a in order_book.get("asks", [])]
    ...
    async def get_historical_candles(
        ...
        response = await self._make_request("GET", "/api/market/candles", params=params)
    ...
    async def place_order(self, order: Order) -> Order:
        ...
        if market_id is None:
            raise InvalidSymbolError(
                f"Unknown symbol: {order.symbol}", exchange=self.exchange_name, symbol=order.symbol
            )
        ...
        client_order_index = (
            int(order.client_order_id) if order.client_order_id else int(time.time())
        )
        ...
        tx_data = await self._sign_transaction(LighterTransactionType.CREATE_ORDER, order_data)

        response = await self._make_request(
            "POST",
            "/api/transaction/send_tx",
            data={"tx_type": LighterTransactionType.CREATE_ORDER, "tx_info": tx_data},
            signed=True,
        )
    ...
    async def _sign_transaction(self, tx_type: int, tx_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Create payload
            payload = json.dumps(
                {"type": tx_type, "info": tx_info}, sort_keys=True, separators=(",", ":")
            )
            ...
            raise AuthenticationError(
                f"Transaction signing failed: {e}", exchange=self.exchange_name
            )
    ...
    async def cancel_order(self, order_id: str, symbol: Optional[str] = None) -> bool:
        ...
        if market_id is None:
            raise InvalidSymbolError(
                f"Symbol required for cancel: {symbol}", exchange=self.exchange_name, symbol=symbol
            )
        ...
        tx_data = await self._sign_transaction(LighterTransactionType.CANCEL_ORDER, cancel_data)

        await self._make_request(
            "POST",
            "/api/transaction/send_tx",
            data={"tx_type": LighterTransactionType.CANCEL_ORDER, "tx_info": tx_data},
            signed=True,
        )
    ...
    async def get_order_status(self, order_id: str, symbol: Optional[str] = None) -> Order:
        ...
        """
        params = {"account_index": self.account_index, "order_index": int(order_id)}
        ...
        response = await self._make_request("GET", "/api/order", params=params, signed=True)
        ...
        symbol = self._market_id_to_symbol.get(
            order_data.get("market_index", 0), symbol or "UNKNOWN"
        )
    ...
    async def get_open_orders(self, symbol: Optional[str] = None) -> List[Order]:
        ...
        response = await self._make_request(
            "GET", "/api/account/orders", params=params, signed=True
        )
    ...
    async def subscribe_to_user(self, callback: Callable[[Dict[str, Any]], None]) -> None:
    ...
    async def _ws_subscribe(
        self, channel: str, callback: Callable[[Dict[str, Any]], None], auth: Optional[str] = None
    ) -> None:
        ...
        if not self._ws_running.get(channel):
            task = asyncio.create_task(self._ws_connection_loop(channel, auth))
    ...
    async def _ws_connection_loop(self, channel: str, auth: Optional[str] = None) -> None:
    ...
    async def _handle_ws_message(self, channel: str, data: Dict[str, Any]) -> None:
