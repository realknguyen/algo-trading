...
def register_adapter(name: str):
    ...
    def decorator(cls: Type[T]) -> Type[T]:
        ...
        if not issubclass(cls, BaseExchangeAdapter):
            raise TypeError(
                f"Adapter class {cls.__name__} must inherit from BaseExchangeAdapter"
            )
...
class ExchangeFactory:
    ...
    def create(exchange: str, **kwargs) -> BaseExchangeAdapter:
        ...
        if missing:
            raise TypeError(
                f"Missing required parameters for {exchange}: {', '.join(missing)}"
            )
    ...
    def create_from_env(
        ...
        if sandbox_var and sandbox_var in os.environ:
            config["sandbox"] = os.environ[sandbox_var].lower() in (
                "true", "1", "yes", "on"
            )
