...
class BinanceAdapter(BaseExchangeAdapter):
                    ...
                    if response.status_code == 200:
                        ...
                        if isinstance(result, dict) and "code" in result and result["code"] < 0:
                            raise BinanceAPIError(
                                result.get("msg", "Unknown error"), result["code"]
                            )
                        return result
    ...
    async def get_balances(self) -> List[Balance]:
        ...
        for asset in account.get("balances", []):
            ...
            if free > 0 or locked > 0:
                balances.append(
                    Balance(asset=asset["asset"], free=free, locked=locked, total=free + locked)
                )
    ...
    async def _ws_connect_with_retry(
        self, stream_name: str, callback: Callable, is_combined: bool = False
    ) -> None:
    ...
    async def subscribe_to_trades(self, symbol: str, callback: Callable) -> str:
        ...
        task = asyncio.create_task(self._ws_connect_with_retry(stream_name, _trade_callback))
        self._ws_tasks[stream_name] = task

        return stream_name

    async def subscribe_to_orderbook(
        self, symbol: str, callback: Callable, depth: int = 100
    ) -> str:
        ...
        task = asyncio.create_task(self._ws_connect_with_retry(stream_name, _orderbook_callback))
        self._ws_tasks[stream_name] = task
    ...
    async def subscribe_to_klines(self, symbol: str, interval: str, callback: Callable) -> str:
        ...
        task = asyncio.create_task(self._ws_connect_with_retry(stream_name, _kline_callback))
        self._ws_tasks[stream_name] = task
    ...
    async def subscribe_to_ticker(self, symbol: str, callback: Callable) -> str:
        ...
        task = asyncio.create_task(self._ws_connect_with_retry(stream_name, _ticker_callback))
        self._ws_tasks[stream_name] = task
    ...
    async def test_order(self, order: Order) -> bool:
        ...
        try:
            await self._make_request("POST", "/api/v3/order/test", params=params, signed=True)
            return True
        except Exception:
            return False
...
if __name__ == "__main__":
    ...
    async def test_adapter():
        ...
        adapter = BinanceAdapter(api_key="test_key", api_secret="test_secret", sandbox=True)
