...
class BybitAdapter(BaseExchangeAdapter):
    ...
    def _generate_signature(
        self, timestamp: str, api_key: str, recv_window: str, params: str
    ) -> str:
        ...
        signature = hmac.new(
            bytes(self._api_secret, "utf-8"), bytes(param_str, "utf-8"), hashlib.sha256
        ).hexdigest()
        return signature

    def _get_auth_headers(
        self, params: Optional[Union[Dict, str]] = None, is_json: bool = False
    ) -> Dict[str, str]:
        ...
        if params is None:
            ...
            param_str = (
                params if isinstance(params, str) else json.dumps(params, separators=(",", ":"))
            )
        else:
                ...
                async with self._rate_limiter:
                    ...
                    if result.get("retCode") != 0:
                        ...
                        if error_code == 10001:  # Params error
                            ...
                            10003,
                            10004,
                        ]:  # Invalid request, IP banned, wrong sign
    ...
    async def get_recent_trades(
        self, symbol: str, category: str = "linear", limit: int = 500
    ) -> List[Dict[str, Any]]:
    ...
    async def set_leverage(
        ...
        response = await self._make_request(
            "POST", "/v5/position/set-leverage", data=data, signed=True
        )
    ...
    async def place_order(self, order: Order, category: str = "linear", **kwargs) -> Order:
    ...
    async def get_open_orders(
        ...
        """
        params = {"category": category, "openOnly": 0}  # Query open orders only
    ...
    async def _ws_connect(
        self, url: str, callback: Callable[[Dict[str, Any]], None], is_private: bool = False
        ...
        while self._ws_running.get(ws_id, False):
            ...
            if self._ws_running.get(ws_id, False):
                ...
                self._logger.info(
                    f"Reconnecting to {url} in {total_delay:.1f}s (attempt {attempts + 1})"
                )
    ...
    def _get_ws_auth_message(self) -> Dict[str, Any]:
        ...
        return {"op": "auth", "args": [self._api_key, timestamp, signature]}
    ...
    async def subscribe_to_position(self, callback: Callable[[Dict[str, Any]], None]) -> str:
    ...
    async def subscribe_to_execution(self, callback: Callable[[Dict[str, Any]], None]) -> str:
    ...
    async def subscribe_to_wallet(self, callback: Callable[[Dict[str, Any]], None]) -> str:
...
if __name__ == "__main__":
    ...
    async def test_adapter():
        ...
        adapter = BybitAdapter(api_key="test_key", api_secret="test_secret", sandbox=True)
