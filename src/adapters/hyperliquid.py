...
class HyperliquidAdapter(BaseExchangeAdapter):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.wallet_address = api_key.lower() if api_key.startswith("0x") else f"0x{api_key.lower()}"
    ...
    def _name_to_asset(self, name: str) -> int:
        ...
        if name not in self._coin_to_asset:
            raise InvalidSymbolError(f"Unknown symbol: {name}", exchange=self.exchange_name, symbol=name)
    ...
    async def _post_exchange(
        ...
        payload = {
            ...
            "nonce": nonce,
            "signature": signature,
            "vaultAddress": self.vault_address if action.get("type") not in ["usdClassTransfer", "sendAsset"] else None,
        ...
        try:
            ...
            if user_state is None:
                raise AuthenticationError(
                    "Failed to verify wallet address",
    ...
    async def get_account(self) -> AccountInfo:
        ...
        try:
            user_role = await self._post_info({
                "type": "userRole",
                "user": self.wallet_address
            })
    ...
    async def place_order(self, order: Order) -> Order:
        ...
        if order.order_type == OrderType.MARKET:
            ...
            slippage_mult = (1 + self.default_slippage) if order.side == OrderSide.BUY else (1 - self.default_slippage)
    ...
    async def cancel_order(self, order_id: str, symbol: Optional[str] = None) -> bool:
        ...
        if not symbol:
            raise OrderError(
                "Symbol is required for Hyperliquid cancel_order",
                exchange=self.exchange_name
            )
    ...
    async def get_funding_rate(self, symbol: Optional[str] = None) -> Union[FundingRate, List[FundingRate]]:
            ...
            for i, asset_info in enumerate(meta.get("universe", [])):
                if asset_info.get("name") == symbol and i < len(contexts):
                    ...
                    return Ticker(
                        ...
                        timestamp=time.time(),
                        high_24h=Decimal(str(ctx.get("prevDayPx", "0"))) * Decimal("1.1"),  # Approximate
                        low_24h=Decimal(str(ctx.get("prevDayPx", "0"))) * Decimal("0.9")   # Approximate
                    )
    ...
    async def get_orderbook(self, symbol: str, limit: int = 100) -> OrderBook:
        ...
        for level in levels[0]:
            bids.append([
                Decimal(str(level.get("px", "0"))),
                Decimal(str(level.get("sz", "0")))
            ])
    ...
    async def _subscribe_ws(
        ...
        self._ws_subscriptions[sub_key] = {
    ...
    async def _ws_loop(
        ...
        while self._ws_running.get("main", False):
            ...
            if self._ws_running.get("main", False):
                ...
                logger.info(f"Reconnecting WebSocket in {reconnect_delay}s (attempt {attempts + 1})")
