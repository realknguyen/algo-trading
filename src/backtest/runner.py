    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        if not self.equity_curve.empty:
            ...
            if len(daily_returns) > 1 and daily_returns.std() > 0:
                metrics["sharpe_ratio"] = (daily_returns.mean() / daily_returns.std()) * np.sqrt(
                    252
                )
...
class BacktestRunner:
    ...
    def run(self, strategy, data: pd.DataFrame, symbol: str = "UNKNOWN") -> BacktestResult:
        """
        ...
        for i in range(warmup, len(data)):
            ...
            if signal:
                if signal.action == "buy" and position == 0:
                    ...
                    entry_date = (
                        current.name if isinstance(current.name, datetime) else data.index[i]
                    )
                    position = 1
