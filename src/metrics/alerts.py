...
class AlertManager:
    ...
    def _cleanup_silences(self, now: float) -> None:
        """Clean up expired silences."""
        expired_rules = [
            rid for rid, until in self._silenced_rules.items() if now > until
        ]
        for rid in expired_rules:
            del self._silenced_rules[rid]
        
        expired_alerts = [
            aid for aid, until in self._silenced_alerts.items() if now > until
        ]
    ...
    def _format_alert_message(self, rule: AlertRule, value: float) -> str:
        ...
        template = rule.annotations.get(
            'summary',
            f"{rule.name}: {rule.metric} is {rule.condition} {rule.threshold}"
        )
