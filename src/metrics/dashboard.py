...
class DashboardData:
    ...
    def update_rate_limit_info(
        self, exchange: str, remaining: int, limit: int, reset_time: Optional[float] = None
    ) -> None:
    ...
    async def get_exchange_health(self, exchange: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "healthy": sum(
                    1 for h in self._exchange_health.values() if h.status == HealthStatus.HEALTHY
                ),
                "degraded": sum(
                    1 for h in self._exchange_health.values() if h.status == HealthStatus.DEGRADED
                ),
                "unhealthy": sum(
                    1 for h in self._exchange_health.values() if h.status == HealthStatus.UNHEALTHY
                ),
            },
        }
    ...
    async def get_circuit_breaker_status(self, exchange: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "half_open": sum(
                    1 for s in self._circuit_states.values() if s == CircuitState.HALF_OPEN
                ),
    ...
    async def get_rate_limit_status(self, exchange: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                "avg_utilization": (
                    sum(r.current_usage for r in self._rate_limits.values())
    ...
    async def get_trading_summary(self, period: str = "daily") -> Dict[str, Any]:
    ...
    async def get_order_metrics(self, exchange: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "avg_fill_rate": (
                    sum(o.fill_rate for o in self._order_metrics.values())
                    / len(self._order_metrics)
    ...
    async def _refresh_exchange_health(self, metrics: Dict[str, Any]) -> None:
        ...
        for exchange in all_exchanges:
            # Calculate metrics
            is_connected = any(exchange in k and v > 0 for k, v in active_connections.items())
            ...
            total_errors = sum(v for k, v in request_errors.items() if exchange in k)
            total_requests = sum(
            ...
            total_attempts = sum(v for k, v in connection_attempts.items() if exchange in k)
            successful = sum(
                v for k, v in connection_attempts.items() if exchange in k and "status=success" in k
            )
    ...
    async def _refresh_rate_limits(self, metrics: Dict[str, Any]) -> None:
        ...
        for exchange in self._exchanges:
            ...
            # Get hit count
            hit_count = sum(v for k, v in rate_limit_hits.items() if exchange in k)

            # Get avg wait time
            wait_snapshots = [s for k, s in wait_time.items() if exchange in k and s is not None]
            avg_wait = (
                sum(s.p50 for s in wait_snapshots) / len(wait_snapshots) if wait_snapshots else 0
            )
    ...
    async def _refresh_order_metrics(self, metrics: Dict[str, Any]) -> None:
        ...
        for exchange in self._exchanges:
            # Count orders
            placed = sum(
                v for k, v in orders_placed.items() if exchange in k and "status=success" in k
            )
            filled = sum(v for k, v in orders_filled.items() if exchange in k)
            cancelled = sum(v for k, v in orders_cancelled.items() if exchange in k)
            ...
            # Get latency metrics
            latency_snapshots = [
                s for k, s in placement_latency.items() if exchange in k and s is not None
            ]
    ...
    async def _refresh_connection_metrics(self, metrics: Dict[str, Any]) -> None:
        ...
        for exchange in self._exchanges:
            # Count connections
            active = sum(v for k, v in active_connections.items() if exchange in k)

            # Count attempts
            attempts = sum(v for k, v in connection_attempts.items() if exchange in k)
            successful = sum(
                v for k, v in connection_attempts.items() if exchange in k and "status=success" in k
            )
            failed = sum(
                v for k, v in connection_attempts.items() if exchange in k and "status=failed" in k
            )

            # Count reconnections
            recon_count = sum(v for k, v in reconnections.items() if exchange in k)

            # Count WebSocket messages
            sent = sum(v for k, v in ws_sent.items() if exchange in k)
            received = sum(v for k, v in ws_received.items() if exchange in k)
    ...
    async def _calculate_trading_summary(self, period: str) -> TradingSummary:
        ...
        # Calculate P&L
        total_pnl = sum(v for k, v in pnl.items())
    ...
    def _calculate_system_overview(self) -> SystemOverview:
        ...
        open_circuits = sum(1 for s in self._circuit_states.values() if s == CircuitState.OPEN)

        # Sum active connections and requests
        total_connections = sum(c.active_connections for c in self._connection_metrics.values())
...
def init_dashboard(collector: MetricsCollector, **kwargs) -> DashboardData:
