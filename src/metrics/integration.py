    ...
    for method_name in connection_methods:
        if hasattr(cls, method_name):
            ...
            setattr(
                cls,
                method_name,
                instrument_connection(exchange_name, "rest", coll)(original)
            )
...
class MetricsMiddleware:
    ...
    def on_request(self, request: httpx.Request) -> None:
        ...
        # Start tracking
        asyncio.create_task(
            self.collector.record_request_started(
                self.exchange,
                request.method
            )
        )
        ...
        **kwargs
        ...
        try:
            if original_make_request:
                ...
                result = await BaseExchangeAdapter._make_request(
...
async def setup_metrics_for_exchange(
    adapter,
    exchange_name: Optional[str] = None
) -> None:
