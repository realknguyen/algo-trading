...
class ContextualLogger:
    ...
    @contextmanager
    def context_span(self, span_name: str, metadata: Optional[Dict[str, Any]] = None):
        """
        ...
        with ContextScope(span_name=span_name, metadata=extra_meta) as ctx:
            ...
            try:
                ...
                elapsed = (datetime.utcnow() - start_time).total_seconds() * 1000
                self.exception(
                    f"Span '{span_name}' failed", extra={"elapsed_ms": elapsed, "error": str(e)}
                )
                raise
...
def log_with_context(
    level: int = logging.INFO, msg: str = "", extra: Optional[Dict[str, Any]] = None
) -> None:
    """
...
def setup_logging(
    level: int = logging.INFO, json_format: bool = True, include_context: bool = True
) -> None:
    """
...
def get_logger(name: str, level: int = logging.INFO, json_format: bool = True) -> ContextualLogger:
