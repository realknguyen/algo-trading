...
class KrakenAdapter(BaseExchangeAdapter):
    ...
    def __init__(
        ...
        self.rate_limit = KrakenRateLimit(
            tier=rate_limit_tier, calls_per_3s=calls_per_3s, remaining_calls=calls_per_3s
        )
    ...
    def _sign_request(self, method: str, endpoint: str, params: Dict[str, Any]) -> Dict[str, str]:
        ...
        return {"API-Key": self.api_key, "API-Sign": base64.b64encode(signature).decode()}
    ...
    def _sign_challenge(self, challenge: str) -> str:
        ...
        secret_bytes = base64.b64decode(self.api_secret)
        signature = hmac.new(secret_bytes, challenge.encode("utf-8"), hashlib.sha512).digest()
    ...
    async def _make_private_request(
        ...
        response = await self._rate_limited_request("POST", endpoint, data=params, signed=True)
    ...
    async def _make_public_request(
        ...
        response = await self._rate_limited_request("GET", endpoint, params=params)
    ...
    async def get_balances(self) -> List[Balance]:
        ...
        for asset, data in result.items():
            ...
            if total > 0:
                balances.append(Balance(asset=clean_asset, free=free, locked=hold, total=total))
    ...
    async def get_orderbook(self, symbol: str, limit: int = 100) -> Dict[str, Any]:
        ...
        params = {"pair": kraken_symbol, "count": min(limit, 500)}
    ...
    async def get_recent_trades(
        self, symbol: str, since: Optional[int] = None, limit: int = 1000
    ) -> List[Dict[str, Any]]:
    ...
    def _convert_time_in_force(self, tif: TimeInForce) -> Optional[str]:
        ...
        """
        mapping = {TimeInForce.GTC: "GTC", TimeInForce.IOC: "IOC", TimeInForce.FOK: "FOK"}
            ...
            for pos_id, pos_data in result.items():
                ...
                positions.append(
                    Position(
                        ...
                        quantity=vol,
                        avg_entry_price=(
                            Decimal(pos_data.get("cost", 0)) / vol if vol > 0 else Decimal("0")
                        ),
    ...
    async def get_historical_candles(
        ...
        params = {"pair": kraken_symbol, "interval": interval_minutes}
    ...
    async def _connect_ws(
        self, url: str, private: bool = False
    ...
    async def _ws_authenticate(self, ws: websockets.WebSocketClientProtocol) -> None:
        ...
        challenge_payload = {"event": "challenge", "api_key": self.api_key}
    ...
    async def _ws_loop(self, ws: websockets.WebSocketClientProtocol, private: bool = False) -> None:
        ...
        while self._ws_running:
            try:
                ...
                self.logger.logger.warning(
                    f"WebSocket {ws_type} connection closed, reconnecting..."
                )
        ...
        try:
            ...
            if isinstance(data, list) and len(data) >= 2:
                ...
                if len(data) >= 4:
                    ...
                    channel_name = self._ws_subscriptions.get(str(channel_id), {}).get(
                        "name", "unknown"
                    )
                    pair = ""
                ...
                callback = self._ws_callbacks.get(callback_key) or self._ws_callbacks.get(
                    channel_name
                )
    ...
    async def subscribe_to_ticker(
        self, symbol: str, callback: Callable[[Dict, str, str], None]
    ) -> None:
    ...
    async def subscribe_to_orderbook(
        self, symbol: str, callback: Callable[[Dict, str, str], None], depth: int = 10
    ) -> None:
        ...
        self._ws_subscriptions[sub_id] = {"name": "book", "payload": subscription, "private": False}
        self._ws_callbacks[sub_id] = callback
    ...
    async def subscribe_to_ohlc(
        self, symbol: str, callback: Callable[[Dict, str, str], None], interval: int = 1
    ) -> None:
        ...
        self._ws_subscriptions[sub_id] = {"name": "ohlc", "payload": subscription, "private": False}
        self._ws_callbacks[sub_id] = callback
