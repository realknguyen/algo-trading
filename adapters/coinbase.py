...
class CoinbaseAdvancedTradeAdapter(BaseExchangeAdapter):
    ...
    def _parse_order_response(self, order_data: Dict[str, Any]) -> Order:
        ...
        return Order(
            ...
            quantity=Decimal(order_data.get('order_configuration', {}).get('limit_limit_gtc', {}).get('base_size', 0)
                          or order_data.get('order_configuration', {}).get('market_market_ioc', {}).get('quote_size', 0)
                          or order_data.get('filled_size', 0)),
            price=Decimal(order_data.get('order_configuration', {}).get('limit_limit_gtc', {}).get('limit_price', 0)
                         or order_data.get('order_configuration', {}).get('limit_limit_gtd', {}).get('limit_price', 0)
                         or 0) or None,
