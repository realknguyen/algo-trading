        ...
        if not self.equity_curve.empty:
            ...
            if len(daily_returns) > 1:
                ...
                if len(downside_returns) > 0 and downside_returns.std() > 0:
                    self.sortino_ratio = (daily_returns.mean() / downside_returns.std()) * np.sqrt(252)
                ...
                if self.max_drawdown_pct != 0:
                    annual_return = ((float(self.final_capital) / float(self.initial_capital)) ** 
                                   (252 / len(daily_returns)) - 1) * 100
...
class BacktestEngine:
    ...
    async def run(
        ...
        self.logger.logger.info(f"Backtest completed: {result.num_trades} trades, {result.total_return_pct:.2f}% return")
    ...
    async def _execute_signal(self, signal: Signal, data: Dict[str, pd.DataFrame]) -> None:
        ...
        if side == OrderSide.BUY:
            ...
            # Calculate P&L
            if position['side'] == OrderSide.BUY:
                pnl = (execution_price - position['entry_price']) * sell_quantity - commission_amount
            else:
                pnl = (position['entry_price'] - execution_price) * sell_quantity - commission_amount
            
            return_pct = float(pnl / (position['entry_price'] * sell_quantity) * 100) if position['entry_price'] > 0 else 0
    ...
    def _close_all_positions(self, final_time: datetime, data: Dict[str, pd.DataFrame]) -> None:
        ...
        for symbol in list(self.positions.keys()):
            ...
            if symbol in data and not data[symbol].empty:
                ...
                commission_amount = execution_price * position['quantity'] * Decimal(str(self.commission))
                
                # Calculate P&L
                if position['side'] == OrderSide.BUY:
                    pnl = (execution_price - position['entry_price']) * position['quantity'] - commission_amount
                else:
                    pnl = (position['entry_price'] - execution_price) * position['quantity'] - commission_amount
