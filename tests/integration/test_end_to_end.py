...
class TestEndToEndWorkflow:
    ...
    def test_strategy_with_risk_integration(self, sample_data):
        ...
        # Setup risk manager
        limits = RiskLimits(max_position_size=0.20, max_drawdown_pct=0.20, daily_loss_limit=5000.0)
        risk_manager = RiskManager(limits, initial_capital=100000)

        # Setup strategy
        strategy = SmaCrossoverRiskStrategy(
            {"fast_period": 10, "slow_period": 30, "stop_loss_pct": 2.0}
        )
        ...
        for i in range(50, len(sample_data)):
            ...
            if signal:
                ...
                valid, reason = risk_manager.validate_order(
                    signal.symbol, quantity=100, price=signal.price  # Simplified
                )
    ...
    def test_full_backtest_pipeline(self, sample_data):
        ...
        # Setup components
        strategy = SmaCrossoverRiskStrategy({"fast_period": 10, "slow_period": 30})
    ...
    def test_execution_with_mock_broker(self, sample_data):
        ...
        risk_manager = RiskManager(RiskLimits(), initial_capital=100000)
        engine = ExecutionEngine(broker=mock_broker, risk_manager=risk_manager)

        # Create order from signal
        order = Order(symbol="TEST", side=OrderSide.BUY, quantity=10, order_type=OrderType.MARKET)

        # Submit order
        order_id = engine.submit_order(order, reference_price=150.0)
