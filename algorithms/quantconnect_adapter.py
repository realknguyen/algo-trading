...
class QCAlgorithmInterface(ABC):
    ...
    def market_order(self, symbol: str, quantity: float) -> None:
        ...
        """
        self._orders.append(
            {"type": "market", "symbol": symbol, "quantity": quantity, "timestamp": self.time}
        )
    ...
    def liquidate(self, symbol: Optional[str] = None) -> None:
        ...
        """
        self._orders.append({"type": "liquidate", "symbol": symbol, "timestamp": self.time})
...
class QuantConnectAdapter(BaseAlgorithm):
    ...
    def __init__(self, qc_algorithm: QCAlgorithmInterface, config: QCAlgorithmConfig, **kwargs):
    ...
    def on_data(self, data: Dict[str, pd.DataFrame]) -> Optional[Signal]:
        ...
        self.qc_algorithm.time = (
            latest_time if isinstance(latest_time, datetime) else datetime.now()
        )
    ...
    def _detect_signal_from_orders(self) -> Optional[Signal]:
        ...
        if order_type == "set_holdings":
            ...
            portfolio_value = (
                self.qc_algorithm.portfolio.total_portfolio_value
                if self.qc_algorithm.portfolio
                else 0
            )
    ...
    async def on_execute(self, signal: Signal) -> Optional[Any]:
        ...
        order = await self.execute_trade(
            symbol=signal.symbol, side=side, quantity=signal.quantity, order_type=signal.order_type
        )
    ...
    async def on_order_filled(self, order: Order) -> None:
        ...
        self.logger.logger.info(
            f"QC order filled: {order.symbol} {order.side.value} {order.filled_quantity}"
        )
