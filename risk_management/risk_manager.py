...
class PositionRisk:
    ...
    def calculate(self) -> None:
        ...
        self.unrealized_pnl_pct = (
            float((self.current_price - self.entry_price) / self.entry_price * 100)
...
class PortfolioRisk:
    ...
    def calculate(self) -> None:
        ...
        self.exposure_pct = (
            float(self.total_exposure / self.current_equity * 100)
            if self.current_equity > 0
            else 0.0
        )
        self.daily_pnl_pct = (
            float(self.daily_pnl / self.total_capital * 100) if self.total_capital > 0 else 0.0
        )
...
class RiskManager:
    ...
    def __init__(self, limits: RiskLimits, initial_capital: Decimal):
        self.limits = limits
    ...
    def _check_circuit_breakers(self) -> None:
        ...
        if self.daily_pnl < -Decimal(str(self.limits.daily_loss_limit)):
            self._trigger_circuit_breaker(f"Daily loss limit breached: ${abs(self.daily_pnl):.2f}")
            return
    ...
    def check_trade_risk(
        ...
        if position_pct > self.limits.max_position_size_pct:
            return (
                False,
                f"Position size {position_pct:.2%} exceeds max {self.limits.max_position_size_pct:.2%}",
            )
        ...
        if exposure_pct > self.limits.max_total_exposure_pct:
            return (
                False,
                f"Total exposure {exposure_pct:.2%} exceeds max {self.limits.max_total_exposure_pct:.2%}",
            )
        ...
        if stop_loss_price:
            ...
            if risk_pct > self.limits.max_risk_per_trade_pct:
                return (
                    False,
                    f"Risk per trade {risk_pct:.2f}% exceeds max {self.limits.max_risk_per_trade_pct:.2f}%",
                )

        return True, None

    def calculate_position_size(
        self, entry_price: Decimal, stop_loss_price: Decimal, risk_pct: Optional[float] = None
    ) -> Decimal:
    ...
    def calculate_stop_loss(
        self, entry_price: Decimal, side: OrderSide, stop_loss_pct: Optional[float] = None
    ) -> Decimal:
    ...
    def calculate_take_profit(
        self, entry_price: Decimal, side: OrderSide, take_profit_pct: Optional[float] = None
    ) -> Decimal:
